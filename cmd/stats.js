...
function formatNumber(number) {return Number(number).toLocaleString("de-DE")}
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const response  = await api.player(playerId);
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription("General player statistics from the creator playtest.")
                .addFields(
                    {
            ...
            let message ="<:support:1520804207060586516> An error occurred!\nCheck if the PlayerID is from the Creator Playtest!\n-# Global support coming with PA6";

            if (error.apiData?.error === "Profile is private") 
               {message = "<:Tutle:1520804017192570880> This player's profile is private."}

            else if (error.apiData?.error)
